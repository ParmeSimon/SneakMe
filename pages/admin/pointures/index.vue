...
import '~/assets/css/pointures.css'
...
const selectedPointure = ref({ id: null, label: '' })
...
const addPointure = async (pointure) => {
  ...
  try {
    const response = await fetch('http://localhost/SneakMe/api/pointures.php', {
      ...
      body: JSON.stringify({ label: pointure.label })
    })
...
const updatePointure = async (pointure) => {
  ...
  try {
    const response = await fetch('http://localhost/SneakMe/api/pointures.php', {
      ...
      body: JSON.stringify({ id: pointure.id, label: pointure.label })
    })
...
const openAddModal = () => {
  selectedPointure.value = { id: null, label: '' }
