...
const selectedCouleur = ref({ id: null, nom: '' })
...
const addCouleur = async (couleur) => {
  ...
  try {
    const response = await fetch('http://localhost/SneakMe/api/couleurs.php', {
      ...
      body: JSON.stringify({ nom: couleur.nom })
    })
...
const updateCouleur = async (couleur) => {
  ...
  try {
    const response = await fetch('http://localhost/SneakMe/api/couleurs.php', {
      ...
      body: JSON.stringify({ id: couleur.id, nom: couleur.nom })
    })
...
const openAddModal = () => {
  selectedCouleur.value = { id: null, nom: '' }
